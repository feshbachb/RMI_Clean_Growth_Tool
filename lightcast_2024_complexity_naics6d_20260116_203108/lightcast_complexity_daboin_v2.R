...
# VERSION 2.2 - WITH GITHUB EXPORT AND GEOMETRY DATA
...
#
# VERSION 2.2 UPDATES:
# - Geometry data frames for all geographic levels (county, state, CBSA, CSA, CZ)
# - Automated export to GitHub repository with timestamped folders
# - Parquet/geoparquet export for all data frames
# - Top 10 ECI/ICI rankings printed for each geographic level
# - Data frame inventory with dimensions and descriptions
#
# VERSION 2.1 UPDATES:
...
library(sfarrow)     # For geoparquet export
library(httr)        # For GitHub API
...
# Create state-level geometry by dissolving counties
state_geometry <- tigris_county_state_cbsa_csa_cz_2024 %>%
  group_by(state_fips, state_name, state_abbreviation) %>%
  summarise(geometry = st_union(geometry), .groups = "drop") %>%
  st_as_sf()

# Create CBSA-level geometry by dissolving counties
cbsa_geometry <- tigris_county_state_cbsa_csa_cz_2024 %>%
  filter(!is.na(cbsa_geoid)) %>%
  group_by(cbsa_geoid, cbsa_name) %>%
  summarise(geometry = st_union(geometry), .groups = "drop") %>%
  st_as_sf()

# Create CSA-level geometry by dissolving counties
csa_geometry <- tigris_county_state_cbsa_csa_cz_2024 %>%
  filter(!is.na(csa_geoid)) %>%
  group_by(csa_geoid, csa_name) %>%
  summarise(geometry = st_union(geometry), .groups = "drop") %>%
  st_as_sf()

# Create Commuting Zone geometry by dissolving counties
cz_geometry <- tigris_county_state_cbsa_csa_cz_2024 %>%
  filter(!is.na(commuting_zone_geoid)) %>%
  group_by(commuting_zone_geoid, commuting_zone_name) %>%
  summarise(geometry = st_union(geometry), .groups = "drop") %>%
  st_as_sf()
...
log_msg("  Geometries: county=", nrow(county_geometry), " state=", nrow(state_geometry),
        " CBSA=", nrow(cbsa_geometry), " CSA=", nrow(csa_geometry), " CZ=", nrow(cz_geometry), "\n")
...
data_frame_inventory <- tibble(
  ...
  Description = c(
    ...
    # Geometry data
    "County boundaries (sf/geoparquet)",
    "State boundaries (sf/geoparquet)",
    "CBSA boundaries (sf/geoparquet)",
    "CSA boundaries (sf/geoparquet)",
    "Commuting zone boundaries (sf/geoparquet)",
  ...
  Type = c(
    ...
    "data.frame", "data.frame", "data.frame", "data.frame", "data.frame", "data.frame",
    # Lookups
    "data.frame", "data.frame", "data.frame", "data.frame", "data.frame", "data.frame", "data.frame",
    # Combined
...
# =============================================================================
# STEP 14: EXPORT TO GITHUB REPOSITORY
# =============================================================================

log_step(14, "Exporting to GitHub Repository")

# ┌─────────────────────────────────────────────────────────────────────────────┐
# │ EXPORT CONFIGURATION                                                        │
# └─────────────────────────────────────────────────────────────────────────────┘
# Skip files that are very large but regenerable by running this script
SKIP_REGENERABLE_LARGE_FILES <- TRUE  # Skip lightcast_2024_data, lightcast_with_geodata

# Simplify geometry to reduce file size (tolerance in degrees, ~0.001 = ~100m)
SIMPLIFY_GEOMETRY <- TRUE
SIMPLIFY_TOLERANCE <- 0.001

# Maximum file size (MB) for direct GitHub upload; larger files go to Release
MAX_FILE_SIZE_MB <- 50

# Files to skip (regenerable from source data)
SKIP_FILES <- if (SKIP_REGENERABLE_LARGE_FILES) {
  c("lightcast_2024_data", "lightcast_with_geodata")
} else {
  c()
}
...
if (github_token == "") {
  cat("  ⚠ No GitHub PAT found. Skipping GitHub export.\n")
  cat("  Set GITHUB_PAT environment variable to enable export.\n")
} else {
  ...
  cat("  Skip large regenerable files:", SKIP_REGENERABLE_LARGE_FILES, "\n")
  cat("  Simplify geometry:", SIMPLIFY_GEOMETRY, "(tolerance:", SIMPLIFY_TOLERANCE, ")\n")
  
  # Use temp directory for staging (cleaned up after upload)
  local_export_dir <- file.path(tempdir(), export_folder)
  dir.create(local_export_dir, showWarnings = FALSE, recursive = TRUE)
  
  # File size threshold configured above
  large_files <- list()
  
  # Helper function to upload file to GitHub Contents API (for small files)
  upload_to_github <- function(local_path, github_path, message) {
    file_size_mb <- file.info(local_path)$size / (1024 * 1024)
    
    if (file_size_mb > MAX_FILE_SIZE_MB) {
      # Track large files for release upload
      large_files[[basename(local_path)]] <<- list(
    ...
    check_url <- paste0("https://api.github.com/repos/", owner, "/", repo, "/contents/", github_path)
    ...
    if (httr::status_code(check_response) == 200) {
      existing <- httr::content(check_response)
    ...
    return(httr::status_code(response) %in% c(200, 201))
  }
  
  # Helper function to create a GitHub Release
  create_github_release <- function(tag_name, release_name, body_text) {
    release_url <- paste0("https://api.github.com/repos/", owner, "/", repo, "/releases")
    ...
    if (httr::status_code(response) %in% c(200, 201)) {
      ...
      # Check if tag already exists, get existing release
      get_url <- paste0("https://api.github.com/repos/", owner, "/", repo, "/releases/tags/", tag_name)
  ...
  # Helper function to upload asset to a GitHub Release (supports files up to 2GB)
  upload_release_asset <- function(release_upload_url, local_path, filename) {
    # Remove {?name,label} from upload URL
    upload_url <- gsub("\\{.*\\}", "", release_upload_url)
    upload_url <- paste0(upload_url, "?name=", URLencode(filename))
    
    cat("      Uploading to release:", filename, "(", 
        round(file.info(local_path)$size / (1024*1024), 1), "MB)...\n")
  ...
  # Export data frames to parquet
  cat("\n  Exporting data frames to parquet...\n")
  if (length(SKIP_FILES) > 0) {
    cat("    (Skipping:", paste(SKIP_FILES, collapse = ", "), ")\n")
  }
  ...
  for (i in 1:nrow(data_frame_inventory)) {
    obj_name <- data_frame_inventory$Object[i]
    obj_type <- data_frame_inventory$Type[i]
    
    # Skip configured large regenerable files
    if (obj_name %in% SKIP_FILES) {
      export_skipped <- export_skipped + 1
      next
    }
    
    if (!exists(obj_name)) {
      cat("    ⚠ Skipping", obj_name, "(not found)\n")
    ...
    tryCatch({
      if (obj_type == "sf") {
        ...
        if (SIMPLIFY_GEOMETRY) {
          obj <- st_simplify(obj, dTolerance = SIMPLIFY_TOLERANCE, preserveTopology = TRUE)
        }
        # Export as geoparquet
        local_file <- file.path(local_export_dir, paste0(obj_name, ".geoparquet"))
        sfarrow::st_write_parquet(obj, local_file)
        github_path <- paste0(export_folder, "/", obj_name, ".geoparquet")
      } else {
        # Export as parquet
        local_file <- file.path(local_export_dir, paste0(obj_name, ".parquet"))
        arrow::write_parquet(obj, local_file)
        github_path <- paste0(export_folder, "/", obj_name, ".parquet")
      }
      
      # Upload to GitHub
      result <- upload_to_github(local_file, github_path, paste("Add", obj_name))
      if (identical(result, "LARGE")) {
        cat("    ◐", obj_name, "(", round(file.info(local_file)$size/(1024*1024), 1), "MB -> Release)\n")
      } else if (result) {
        cat("    ✓", obj_name, "\n")
        export_success <- export_success + 1
      } else {
        cat("    ✗", obj_name, "(upload failed)\n")
        export_failed <- export_failed + 1
      }
    }, error = function(e) {
      cat("    ✗", obj_name, "(error:", conditionMessage(e), ")\n")
      export_failed <<- export_failed + 1
    })
  }
  
  # Export complexity result components
  cat("\n  Exporting complexity results...\n")
  
  for (level in c("county", "state", "cbsa", "csa", "cz")) {
    complexity_obj <- get(paste0(level, "_complexity"))
    
    for (component in c("eci", "ici", "diversity", "ubiquity", "K_c1", "K_i1")) {
      if (!is.null(complexity_obj[[component]])) {
        obj_name <- paste0(level, "_", component)
        local_file <- file.path(local_export_dir, paste0(obj_name, ".parquet"))
        
        tryCatch({
          arrow::write_parquet(complexity_obj[[component]], local_file)
          github_path <- paste0(export_folder, "/complexity/", obj_name, ".parquet")
          result <- upload_to_github(local_file, github_path, paste("Add", obj_name))
          if (identical(result, "LARGE")) {
            # Large complexity files are rare but tracked
          } else if (result) {
            cat("    ✓", obj_name, "\n")
            export_success <- export_success + 1
          }
  ...
  # Export peer relationships
  cat("\n  Exporting peer relationships...\n")
  
  for (level in c("county", "cbsa")) {
    peer_obj <- get(paste0(level, "_peers"))
    
    if (!is.null(peer_obj$peer_relationships)) {
      obj_name <- paste0(level, "_peer_relationships")
      local_file <- file.path(local_export_dir, paste0(obj_name, ".parquet"))
      
      tryCatch({
        arrow::write_parquet(peer_obj$peer_relationships, local_file)
        github_path <- paste0(export_folder, "/peers/", obj_name, ".parquet")
        result <- upload_to_github(local_file, github_path, paste("Add", obj_name))
        if (identical(result, "LARGE")) {
          # Large peer files tracked
        } else if (result) {
          cat("    ✓", obj_name, "\n")
          export_success <- export_success + 1
        }
  ...
  # Export a copy of this R script
  cat("\n  Exporting R script...\n")
  
  script_source <- NULL
  possible_paths <- c(
    file.path(onedrive_base, "US Program - Documents/6_Projects/Clean Regional Economic Development/ACRE/Data/Data_Explorer_2026_Prep/Data_Explorer_2026/Lightcast.R"),
    "Lightcast.R"
  )
  ...
  if (!is.null(script_source) && file.exists(script_source)) {
    local_script <- file.path(local_export_dir, "lightcast_complexity_daboin_v2.R")
    file.copy(script_source, local_script, overwrite = TRUE)
    github_path <- paste0(export_folder, "/lightcast_complexity_daboin_v2.R")
    result <- upload_to_github(local_script, github_path, "Add R script")
    if (result && !identical(result, "LARGE")) {
      cat("    ✓ R script exported\n")
      export_success <- export_success + 1
    }
  } else {
    cat("    ⚠ R script not found for export\n")
  }
  
  # Export README
  large_file_list <- if (length(large_files) > 0) {
    paste0("\n\n## Large Files (in GitHub Release)\n",
           "The following files exceed GitHub's ", MAX_FILE_SIZE_MB, "MB limit and are available in the Release assets:\n",
           paste("- ", sapply(large_files, function(x) paste0(x$filename, " (", x$size_mb, " MB)")), collapse = "\n"))
  } else {
    ""
  }
  
  skip_file_list <- if (length(SKIP_FILES) > 0) {
    paste0("\n\n## Regenerable Files (Not Included)\n",
           "The following large files can be regenerated by running the R script:\n",
           paste("- ", SKIP_FILES, collapse = "\n"))
  } else {
    ""
  }
  
  geometry_note <- if (SIMPLIFY_GEOMETRY) {
    paste0("\n\n**Note:** Geometry files have been simplified (tolerance: ", SIMPLIFY_TOLERANCE, 
           " degrees, ~", round(SIMPLIFY_TOLERANCE * 111000), "m) to reduce file size.")
  } else {
    ""
  }
  ...
  readme_content <- paste0(
    ...
    "## Contents\n\n",
    "### Data Frames (Parquet)\n",
    paste(setdiff(data_frame_inventory$Object, SKIP_FILES), collapse = ", "), "\n\n",
    "### Complexity Results\n",
    ...
    "## Geographic Levels\n",
    "- County (", nrow(county_names), ")\n",
    "- State (", nrow(state_names), ")\n",
    "- CBSA (", nrow(cbsa_names), ")\n",
    "- CSA (", nrow(csa_names), ")\n",
    "- Commuting Zone (", nrow(cz_names), ")\n\n",
    "## Industries\n",
    "- NAICS 6-digit codes: ", nrow(industry_titles), "\n",
    large_file_list,
    skip_file_list
  )
  ...
  github_path <- paste0(export_folder, "/README.md")
  result <- upload_to_github(local_readme, github_path, "Add README")
  if (result && !identical(result, "LARGE")) {
    cat("    ✓ README.md exported\n")
    export_success <- export_success + 1
  }
  
  # Upload large files to GitHub Release
  if (length(large_files) > 0) {
    cat("\n  Creating GitHub Release for large files (", length(large_files), " files)...\n")
    
    release_tag <- paste0("v", timestamp)
    release_name <- paste0("Lightcast Complexity Data - ", format(Sys.time(), "%Y-%m-%d %H:%M"))
    release_body <- paste0(
      "Large data files for economic complexity analysis.\n\n",
      "**Files included:**\n",
      paste("- ", sapply(large_files, function(x) paste0(x$filename, " (", x$size_mb, " MB)")), collapse = "\n"),
      "\n\n**Folder:** ", export_folder
    )
    
    release <- create_github_release(release_tag, release_name, release_body)
    
    if (!is.null(release)) {
      cat("    ✓ Release created:", release$html_url, "\n")
      
      # Upload each large file as a release asset
      for (file_info in large_files) {
        success <- upload_release_asset(release$upload_url, file_info$local_path, file_info$filename)
        if (success) {
          cat("    ✓", file_info$filename, "uploaded to Release\n")
          export_success <- export_success + 1
        } else {
          cat("    ✗", file_info$filename, "upload failed\n")
          export_failed <- export_failed + 1
        }
      }
      ...
      readme_content <- paste0(readme_content, 
                               ...
                               "Large files are available in the [GitHub Release](", release$html_url, ")")
      writeLines(readme_content, local_readme)
      upload_to_github(local_readme, paste0(export_folder, "/README.md"), "Update README with release link")
      
    } else {
      cat("    ✗ Failed to create release. Large files saved locally only.\n")
  ...
  cat("  EXPORT SUMMARY\n")
  cat("  ════════════════════════════════════════════════════════════════\n")
  cat("  Files uploaded to repo:    ", export_success, "\n")
  if (length(large_files) > 0) {
    cat("  Files uploaded to release: ", length(large_files), "\n")
  }
  if (export_skipped > 0) {
    cat("  Skipped (regenerable):     ", export_skipped, "\n")
  }
  cat("  Failed:                    ", export_failed, "\n")
  cat("  GitHub folder: https://github.com/", owner, "/", repo, "/tree/", branch, "/", export_folder, "\n", sep = "")
  if (length(large_files) > 0) {
    cat("  GitHub release: https://github.com/", owner, "/", repo, "/releases/tag/v", timestamp, "\n", sep = "")
  }
  cat("  ════════════════════════════════════════════════════════════════\n")
  
  # Clean up temp directory
  unlink(local_export_dir, recursive = TRUE)
  
  # ═══════════════════════════════════════════════════════════════════════════
  # STEP 15: DOWNLOAD AND VERIFY TEST
  # ═══════════════════════════════════════════════════════════════════════════
  
  log_step(15, "Download and Verify Test")
  
  # Configuration
  RUN_DOWNLOAD_TEST <- TRUE  # Set to FALSE to skip this test
  
  if (RUN_DOWNLOAD_TEST) {
    
    cat("  Clearing memory...\n")
    
    # List of objects to remove (all data frames we created, but keep essential ones)
    objects_to_remove <- c(
      # Data frames from inventory
      "alaska_pop", "tigris_2024_counties", "tigris_2024_states", "cz",
      "county_state_cbsa_csa_cz_crosswalk",
      "county_geometry", "state_geometry", "cbsa_geometry", "csa_geometry", "cz_geometry",
      "lightcast_2024_data", "lightcast_with_geodata",
      ...
      "validation_results",
      # Complexity objects
      "county_complexity", "state_complexity", "cbsa_complexity", 
      "csa_complexity", "cz_complexity",
      # Peer objects
      "county_peers", "cbsa_peers",
      # Intermediate objects
      "tigris_county_state_cbsa_csa_cz_2024", "counties_2024", "states_2024",
      "cbsa_2024", "csa_2024"
    )
    
    # Remove objects that exist
    ...
    cat("    Removed", length(existing_objects), "objects\n")
    cat("    Memory freed:", round(sum(gc_result[,2]), 1), "MB\n")
    
    # Create temp directory for downloads
    download_dir <- file.path(tempdir(), paste0("verify_", export_folder))
    dir.create(download_dir, showWarnings = FALSE, recursive = TRUE)
    
    # Get list of files in the GitHub folder
    cat("\n  Fetching file list from GitHub...\n")
    
    contents_url <- paste0("https://api.github.com/repos/", owner, "/", repo, 
                           "/contents/", export_folder, "?ref=", branch)
    
    contents_response <- httr::GET(
      contents_url,
      httr::add_headers(
        Authorization = paste("token", github_token),
        Accept = "application/vnd.github.v3+json"
    ...
    if (httr::status_code(contents_response) != 200) {
      cat("    ✗ Failed to fetch file list (status:", httr::status_code(contents_response), ")\n")
    } else {
      files <- httr::content(contents_response)
      
      # Filter to parquet/geoparquet files only (exclude R script and README)
      data_files <- files[sapply(files, function(f) {
        grepl("\\.(parquet|geoparquet)$", f$name)
      })]
      
      cat("    Found", length(data_files), "data files to verify\n")
      
      # Also check subdirectories (complexity, peers)
      subdirs <- files[sapply(files, function(f) f$type == "dir")]
      
      for (subdir in subdirs) {
        subdir_url <- paste0("https://api.github.com/repos/", owner, "/", repo,
                             "/contents/", export_folder, "/", subdir$name, "?ref=", branch)
        subdir_response <- httr::GET(
          subdir_url,
          httr::add_headers(
            Authorization = paste("token", github_token),
            Accept = "application/vnd.github.v3+json"
        ...
        if (httr::status_code(subdir_response) == 200) {
          subdir_files <- httr::content(subdir_response)
          subdir_data_files <- subdir_files[sapply(subdir_files, function(f) {
            grepl("\\.(parquet|geoparquet)$", f$name)
          })]
          # Add subdir path to names
          for (i in seq_along(subdir_data_files)) {
            subdir_data_files[[i]]$subdir <- subdir$name
          }
          data_files <- c(data_files, subdir_data_files)
        }
      }
      
      cat("    Total files (including subdirs):", length(data_files), "\n\n")
      
      # Download and read each file
      cat("  Downloading and verifying files...\n")
      ...
      for (file_info in data_files) {
        ...
        subdir <- if (!is.null(file_info$subdir)) file_info$subdir else ""
        
        # Display name with subdir prefix
        display_name <- if (subdir != "") paste0(subdir, "/", file_name) else file_name
        ...
        tryCatch({
          ...
          if (httr::status_code(download_response) != 200) {
            cat("    ✗", display_name, "(download failed)\n")
            verify_failed <- verify_failed + 1
            next
          }
          
          # Read file based on extension
          if (grepl("\\.geoparquet$", file_name)) {
            df <- sfarrow::st_read_parquet(local_path)
          } else {
            df <- arrow::read_parquet(local_path)
          }
          ...
          cat("    ✓", display_name, "(", format(n_rows, big.mark = ","), " × ", n_cols, ")\n", sep = "")
          verify_success <- verify_success + 1
          
          # Clean up - remove the downloaded object
          rm(df)
          
        }, error = function(e) {
          cat("    ✗", display_name, "(error:", conditionMessage(e), ")\n")
          verify_failed <<- verify_failed + 1
        })
      }
      ...
      cat("  ════════════════════════════════════════════════════════════════\n")
      cat("  Files verified:  ", verify_success, "\n")
      cat("  Failed:          ", verify_failed, "\n")
      if (verify_failed == 0) {
        cat("  Status:          ✓ ALL FILES VERIFIED SUCCESSFULLY\n")
      } else {
        cat("  Status:          ⚠ SOME FILES FAILED VERIFICATION\n")
      }
      cat("  ════════════════════════════════════════════════════════════════\n")
    }
    
  } else {
    cat("  Download test skipped (RUN_DOWNLOAD_TEST = FALSE)\n")
  }
}
